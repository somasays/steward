...
is known -- it establishes the trace and records the run's identity on it, and
it is milliseconds long even for a run that goes on for ten minutes. Calling it
`run` would put a span in the trace that looks like it measures the run and
does not. The task spans a worker opens later sit on the same trace id, as
