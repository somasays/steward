...
4. **Audit against each invariant.** Walk I1–I15 explicitly. For the invariants **GUARDRAILS.md §5 currently lists as review-enforced** — read that list, do not recall it; it shrinks as harnesses land, and it lost masking when H7 went active (#49) — you ARE the enforcement: scrutinize hardest there. Scrutinize an invariant that has *just* acquired a harness just as hard: a check is only as good as the cases it was written against, and the diff that stands review enforcement down is the diff that has to earn it.
   ...
   - I4: new imports — check direction and whether package-to-package edges are declared, in the root `pyproject.toml`'s `[tool.importlinter]` contracts (S1's declaration; `scripts/fitness/boundaries.json` now only holds the `contained_modules` map for S5) **and in the importing package's own `pyproject.toml`, test trees included**. Import-linter's `root_packages` are the `src/` trees, so an undeclared edge in a `tests/` directory is invisible to S1 and resolves transitively until the package it borrowed from drops the dependency — two of #49's rounds found exactly that.
