...
- **`execute`'s bookkeeping connection** is covered, because a task IS claimed by the time it runs — this is exactly the task-scoped answer `_claim` lacks. A short bounded retry (`CONNECT_RETRY_ATTEMPTS = 3`, `CONNECT_RETRY_DELAY = 150ms` — a fraction of a poll interval, nowhere near a task's lease) absorbs the shape #56 actually found reachable: losing a race for the last slot in the pool. If the pool stays out of room past that bound, there is no connection left to write a terminal state on, so the worker does not try to conjure one: `mark_running` never ran, so the row is left `claimed` rather than `running`, unrecorded, for `requeue_stale` to reclaim at lease expiry — the same "recorded by neither context" outcome the paragraph above already accepts, reached from a different seam.
