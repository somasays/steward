...
| **Classifier** | one asset at one profile version | that profile: column metadata, statistics and masked samples | *none* — `submit_result` only | `ColumnClassification` per column: labels (`pii`, `phi`, `financial`, `none`), confidence, and a resolvable evidence reference per sensitive label ([D15](#13-key-design-decisions)) |
...
- **Every run leaves an artifact, and the artifact says whether it may be quoted.** `steward evals run` writes per-column verdicts and per-run disagreement to `evals/artifacts` (git-ignored, `--artifacts` to redirect). It carries the fixture version read from the file, the model alias, a digest of the gateway config, and the pinned proxy image and model revision where they are set. `release_evidence` is **computed** and fails closed. It requires immutably pinned digests (a moving tag is not a pin), `STEWARD_EVALS_REQUIRED=1`, and the responding model on record — the last of which is #89 and is not satisfiable yet, because `litellm.production.yaml` falls `steward-classify` back to `steward-fast` and nothing captures which answered. Nothing sets the flag either: the release job is #88. Never asserted, because: a developer preflight routes to whatever that laptop runs — Ollama, in the committed preflight config — and its scores characterise no deployed model, so an artifact that did not say so would eventually be read as the release result.
...
|---|---|---|
| **M0 — Skeleton** | uv workspace, schemas package, FastAPI app, Postgres migrations, task queue + worker loop (no LLM), CI green | a no-op run flows API → queue → worker → done under **end-to-end trace correlation**: one trace id, generated locally and stored NOT NULL, carried from the POST response through every span and audit row. Verifying the span *tree* is H6, which landed with #69; verifying that Langfuse itself **received** a resolvable trace remains review-enforced, since the assertions cover the spans the tracer seam is handed rather than the export — M0 deliberately runs credential-free |
...
*The four dimensions, and what "enforced" means for each.* Steps, tokens and cost are reserved at planning time and checked again at runtime: a task whose reported usage exceeds its own cap is recorded as `budget_exceeded` and its usage is never rolled up, so `runs.used_*` cannot be walked past `runs.budget_*` one task at a time. That runtime check is the *outer* fence; stopping an agent at the step that would cross its cap is in-loop enforcement, which **landed with #69** and is what H4's step/token/cost half now measures. Wall-clock is reserved the same way and enforced by the worker's deadline (D7), unchanged.
...
*The transport landed, as a client for the proxy rather than an embedded router (#69 review).* The gap below was real and is closed: `ProxyConfig` carries the proxy's address and credential, validated **separately** from the routing table — which is what made the transport unimplementable, since a routing table has no address for the thing doing the routing. Plaintext is refused off loopback, the credential is absent from every representation, exception and serialised form, and `LiteLLMProxyTransport` speaks the proxy's OpenAI-compatible HTTP API: streaming, timeouts, cancellation passed through untyped (D11's overrun proof depends on it), usage extraction and owned error translation. Requests name a **Steward alias** and nothing else, so routing stays the proxy's table. Rejected again, and now for a second reason: *an in-process LiteLLM router* would take a copy of the cost budgets, caching, retry and fallback §6 puts in the proxy, and two copies drift. One protocol limit is handled rather than merely stated: streaming reports usage once at the end, so an interrupted call has no reported figure at all. It is charged the **proven upper bound** the preflight already approved — the prompt ceiling and completion allowance, at the configured prices — because the alternative was a failed call that really spent money being recorded at zero dollars, which is exactly what "debit actual usage, including failed calls" exists to prevent and the direction that lets a run overspend quietly. A completed call is exact; an interrupted one is conservative and cannot exceed a bound the budget had already accepted.
...
*What counts as a breach is decided at the ledger's scale.* `budget_cost_usd` and `used_cost_usd` are both `numeric(14, 6)`, so a run's remaining balance only exists in multiples of `LEDGER_COST_SCALE`, and the charge is compared against it at that scale (`steward_queue.ledger_cost`). Differencing the gateway's unrounded figure against the stored delta instead makes the *seventh* decimal place a budget breach: the first live classification run failed all three attempts that way — a $0.000075 step against a $0.50 cap, steps and tokens applied unclamped — and it failed or passed on whether that decimal happened to round down, which is a correct run failing intermittently. The clamp itself is unaffected; it is `LEAST(used + cost, budget)` in SQL, so I12's "the recorded balance stays inside the budget" never depended on this comparison. The audit row keeps `requested` at full precision so the sub-microdollar residue stays visible rather than being folded into either figure.
...
*One consequence worth stating plainly.* The proof agent's goal is **not** in the shipped registry. `steward-orchestration`'s goal/handler seam check asserts every registered goal plans a task type something can execute, and it caught an `agent_echo` goal whose handler only exists once a worker's composition root opts into a transport — a run a client could create and nothing would ever claim. The goal is therefore registered by the acceptance test that needs it, and the product's first agent goal arrives with the Classifier (#50) — which resolves the tension rather than inheriting it, by splitting the workflow from the model capability ([D15](#13-key-design-decisions)).

**D14 — Approval is one atomic supersession, serialised per asset by an advisory lock.**
Publishing a new classification while an old one is approved has to demote the old and promote the new *together*. The alternative — reject the incumbent, then approve the replacement — makes the operator perform a second action to leave a state nobody asked for: an asset with **no** current classification, visible to every reader in between, and permanent if the second action never happens. So the operator approves a replacement and the supersession is part of what that means (#50).

Order inside the transaction is forced by the schema: a partial unique index allows one `approved` row per asset, so the incumbent is demoted *before* the target is promoted.

*Why an advisory lock on the asset rather than a row lock on the approved proposal.* The row lock is the obvious choice and fails in the case that matters most: on a **first** approval there is no approved row to lock, so two concurrent first approvals each find nothing to contend for and both promote. One then meets the unique index and surfaces a raw `UniqueViolation` — a database error where the product has a decision to report. The lock exists so the loser gets a typed `ClassificationConflict`. This is demonstrated rather than argued: neutralising the lock makes the race test fail with `the loser got UniqueViolation` (PROOFS row 98).

The index remains the **final fence** — it is what makes "one approved version" true even for a caller that never came through the repository. The lock provides understandable product behaviour; the index provides the guarantee.

*Idempotency is scoped to the request, not the key alone.* Approve and reject are opposite governance actions sharing one key index, so a key that settled a different proposal — or the opposite outcome — is refused as `IdempotencyKeyReused` rather than replayed. Without that scoping a `reject` replayed under an `approve`'s key returned the approved record and left the rejected proposal untouched: the caller was told their rejection succeeded while nothing had happened to the proposal they named. Found by review, not by the tests that existed.

*Evidence is resolved against the stored profile, not just the proposal.* The type validator checks that a citation names its own column; only the database knows whether that column was in the profile version the proposal claims to have read. `propose` refuses a citation that resolves to nothing, because an unresolvable citation is indistinguishable, to a reviewer, from a fabricated one.

**D15 — The catalog owns the classification workflow; the model behind it is a capability a worker injects.**
`classify_asset` reads an immutable profile version, decides what a classifier may see, turns what comes back into a `ClassificationProposal`, and persists it as `pending_review`. All of that is catalog state, so the handler is `steward_catalog.classify_handler` and it is registered at import like `scan_source` and `profile_asset`. The one thing that package must not do is call a model — I4 forbids it `steward-agents` and `steward-llm`, and it should, because a package that owns the privacy boundary must not also own a way past it. So the model call sits behind `ColumnClassifier`, a protocol whose whole vocabulary is evidence in and proposed columns out, and `services/workers/classifier.py` implements it with the bounded runtime, the `steward-classify` alias, the reservation, the empty tool allowlist and the versioned prompt.

*This is what makes it a shipped goal.* D13's closing note recorded why `agent_echo`'s goal stayed out of the registry: the seam check requires a registered goal's task types to have handlers reachable by importing packages, and an agent handler needs a gateway only a composition root may validate (I15). Splitting the workflow from the capability resolves that without weakening the check — the handler is registered systemwide, and what a process supplies is the model.

*Registration is systemwide; claiming is per process.* The consequence is that a worker can hold a handler it cannot execute, so the composition root narrows its claim list (`Worker(task_types=...)`) rather than claiming `classify_asset` and failing it. A task nobody can execute sitting in the queue is visible as a backlog; a worker that takes it in order to refuse it looks like a broken classifier. The gate is keyed on the pathology — a map from task type to the predicate that says the capability is present — not on an allowlist of types known to be safe. A worker asked for a transport and left with no classifier bound **refuses to start**, so a process cannot come up healthy while the capability it exists for is absent.

*The classifier is given execution identity, never the handler's transaction.* `ClassificationRun` carries the task and run ids, the fencing pair, the trace id and the budget — what a durable checkpoint and a bounded loop need. `TaskContext` carries all of that plus the catalog's open connection, and handing a model-facing adapter that connection is the privilege this seam exists to withhold.

*The request is the profile, not a projection of it.* A `TableProfile` is already the evidence-only view: every value-carrying field on it is a `MaskedSample` by construction (I6, D10). A hand-written "safe fields" projection would be a second allowlist to keep in sync with the first, and green on the day it fell behind.

*Provenance comes from the adapter, not from the model.* The submitted result carries columns only; `asset_id` and `profile_version` are the handler's, and `prompt_version` and `model_alias` are the adapter's. Asking a model to restate which run it was is asking it to get provenance wrong, or to forge it.

*Rejected: a `packages/steward-classifier`.* It would own the same protocol implementation this module does, need the same composition-root binding, and add a package, an import-linter contract and a contract baseline for it — against SPEC §2's `services/workers/`, which is "one entrypoint per agent type". There is not yet enough independent domain surface to justify one. *Rejected: teaching the goal registry about entry-point-provided task types.* It would let any goal declare its way out of the seam check, weakening a valuable static guarantee to accommodate composition.
