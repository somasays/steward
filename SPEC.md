...
*In-loop enforcement is what keeps a single attempt inside its cap*, and it is now real: every model and tool step reserves its worst case against what is left and is refused before it starts, with the tool half reserving a figure each tool declares rather than the zero that made it an audit fence.
