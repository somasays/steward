...
*Where the bound stops.* Reservation counts each task **once**, so the run's budget bounds one pass of the plan. A task that fails and retries executes again, and **that spend is now recorded** — `tasks.fail` carries usage and rolls it up, which is the change this paragraph used to defer to "the agent loop that will make the numbers non-trivial" (#69, [D12](#13-key-design-decisions)). `runs.used_*` is therefore the sum over *every* attempt, succeeded or not, rather than over succeeded tasks only. It is still not reserved: multiplying the reservation by `max_attempts` was rejected then and is rejected now, because it would triple every advertised budget to defend against spend the accounting can now simply see. So the advertised cap bounds one pass and the recorded figure can legitimately exceed it on a retried run — which is the honest reading, and visible, rather than an undercount that made the cap look respected. One smaller conservatism in the same direction: two planned tasks with identical type and payload reserve twice but dedupe to one row at enqueue, so such a plan reserves more than it can spend.
...
*What this costs.* `runs.used_*` can now legitimately exceed `runs.budget_*` on a retried run, because retries are recorded but not reserved (D9). That is the honest direction: the alternative was a number that stayed inside its cap by not looking at the spend that left it. In-loop enforcement is what keeps a *single* attempt inside its cap, and it is now real — every model and tool step reserves its worst case against what is left and is refused before it starts, with the tool half reserving a figure each tool declares rather than the zero that made it an audit fence.
