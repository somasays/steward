...
* **Zero is a failure, not a pass.** A harness that enumerates nothing and boots
  ...
  signature defect. The count is asserted, every service is asserted to declare
  at least one script, and the number of boots is asserted to equal the number
  of entry points.
* **A control run says the refusal is specific.** Each entry point is booted a
  ...
  refuses everything, or that died of an unset DSN. The environment is built from
  scratch for both runs, so a developer's exported `STEWARD_DEPLOYMENT_MODE`
  cannot quietly turn the refusal off.
"""
...
def boot(target: str, env: dict[str, str], argument: Path) -> subprocess.CompletedProcess[str]:
    """Start one entry point the way its console script does, in a clean environment.

    `sys.executable -c` rather than the installed script: the same interpreter the
    tests run under, with no PATH resolution in between. The argument is passed to
    every entry point uniformly — the ones that ignore argv ignore it, and the
    exporter writes to a temporary file instead of the working tree.
    """
...
def test_every_service_entry_point_refuses_an_off_allowlist_gateway(tmp_path: Path) -> None:
    ...
    for service, script, target in entry_points:
        booted = boot(target, rogue, tmp_path / f"{script}.out")
...
def test_the_committed_config_is_not_refused_by_any_entry_point(tmp_path: Path) -> None:
    ...
    checked: list[str] = []
    for service, script, target in entry_points:
        booted = boot(target, approved, tmp_path / f"{script}.out")
        output = booted.stdout + booted.stderr
        where = f"{service}:{script} ({target})"
